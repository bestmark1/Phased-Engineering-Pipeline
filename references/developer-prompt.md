...
- **No orphan tests.** Every test must name what it proves — a PRD acceptance
  criterion by its ID (`AC-014`), an architecture constraint, or a defect that must
  not return — in the test description or a one-line comment. The ID is what lets
